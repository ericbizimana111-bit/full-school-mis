...
exports.updateGrade = async (req, res, next) => {
    try {
        ...
        grade = await Grade.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true })
            .populate('student')
            .populate('exam')
            .populate('subject');
        ...
        logger.error(`Update Grade Error: ${error.message}`);
        next(error);
    }
};
...
exports.getGradesByExam = async (req, res, next) => {
    try {
        const grades = await Grade.find({ exam: req.params.examId })
            ...
            .sort({ 'marks.obtained': -1 });

        res.status(200).json({ success: true, count: grades.length, data: grades });
    } catch (error) {
        logger.error(`Get Grades By Exam Error: ${error.message}`);
        next(error);
    }
};

// @desc Get report card
// @route GET /api/grades/report/:studentId/:examId
// @access Private
exports.getReportCard = async (req, res, next) => {
    try {
        ...
        logger.error(`Get Report Card Error: ${error.message}`);
        next(error);
    }
};